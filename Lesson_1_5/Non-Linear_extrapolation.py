...
Статистичне навчання:
 - визначення параметрів експоненційної моделі за статистичною дискретною навчальною вибіркою;
...
5. Статистичне навчання параметрів експоненційної моделі за традиційним рішенням - бібліотека scipy;
6. Статистичне навчання параметрів синусно-косинусної моделі за R&D рішенням;
7. Прогнозування динаміки розвитку статистичного ряду за МНК класичним та синусно-косинусної моделі за R&D рішенням.
...
def Model (n):
    S0=np.zeros((n))
    for i in range(n):
        S0[i]=(7.8*mt.cos(0.05*i)+9.5*mt.sin(0.05*i))     # квадратична модель реального процесу
    return S0
...
# ---------------------------  МНК ПРОГНОЗУВАННЯ сінусно-косінусне -------------------------------
def MNK_Extrapol_sin_cos (S0, koef):
    ...
    plt.plot(YMNK, 'g*-', label="MNK класична модель")
    plt.legend()
...
if __name__ == '__main__':
    ...
    print('3 - МНК sin_cos за R&D')
    print('4 - МНК прогнозування sin_cos за R&D')
    mode = int(input('mode:'))
    ...
    if (mode == 3):
        ...
        Stat_characteristics_in(S_AV_Detect_sliding_wind, 'Вибірка очищена від АВ алгоритм sliding_wind')
        StartTime = time.time()  # фіксація часу початку обчислень
        Yout_SV_AV_Detect_sliding_wind = MNK_sin_cos(S_AV_Detect_sliding_wind)
        totalTime = (time.time() - StartTime)  # фіксація часу, на очищення від АВ
        Stat_characteristics_out(SV_AV, Yout_SV_AV_Detect_sliding_wind,
                             'MNK ЕКСПОНЕНТА, вибірка очищена від АВ алгоритм sliding_wind')
        # --------------- Оцінювання якості моделі та візуалізація -------------------------
        r2_score(S_AV_Detect_sliding_wind, Yout_SV_AV_Detect_sliding_wind, 'MNK sin_cos_модель_згладжування')
        print('totalTime =', totalTime, 's')
        Plot_AV(Yout_SV_AV_Detect_sliding_wind, S_AV_Detect_sliding_wind,
                'MNK sin_cos: Вибірка очищена від АВ алгоритм sliding_wind')

    if (mode == 4):
        print('MNK ПРОГНОЗУВАННЯ sin_cos')
        # --------------- Очищення від аномальних похибок sliding window -------------------
        n_Wind = 5  # розмір ковзного вікна для виявлення АВ
        koef_Extrapol = 0.5  # коефіціент прогнозування: співвідношення інтервалу спостереження до  інтервалу прогнозування
        koef = mt.ceil(n * koef_Extrapol)  # інтервал прогнозу по кількісті вимірів статистичної вибірки
        S_AV_Detect_sliding_wind = Sliding_Window_AV_Detect_sliding_wind(SV_AV, n_Wind)
        Stat_characteristics_in(S_AV_Detect_sliding_wind, 'Вибірка очищена від АВ алгоритм sliding_wind')
        Yout_SV_AV_Detect_sliding_wind = MNK_Extrapol_sin_cos(S_AV_Detect_sliding_wind, koef)
        Stat_characteristics_extrapol(koef, Yout_SV_AV_Detect_sliding_wind,
                             'MNK ПРОГНОЗУВАННЯ sin_cos, вибірка очищена від АВ алгоритм sliding_wind')
        Plot_AV(Yout_SV_AV_Detect_sliding_wind, S_AV_Detect_sliding_wind,
                'MNK ПРОГНОЗУВАННЯ sin_cos: Вибірка очищена від АВ алгоритм sliding_wind')
