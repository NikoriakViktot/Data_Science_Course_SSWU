...
class TelegramPeriodRequest(BaseModel):
    country_code: str = Field(..., description="Код країни ('ua', 'bel', 'rus')")
    date_start: str = Field(..., description="Початкова дата у форматі YYYYMMDD")
    date_end: str = Field(..., description="Кінцева дата у форматі YYYYMMDD")
...
@app.post("/download_telegrams_period")
def download_telegrams_period(request: TelegramPeriodRequest):
    """
    Завантажує телеграми для заданої країни за період, який задається початковою та кінцевою датою.

    Приклад запиту:
    ```bash
    curl -X POST "http://localhost:8000/download_telegrams_period" \
      -H "Content-Type: application/json" \
      -d '{"country_code": "ua", "date_start": "20230801", "date_end": "20230831"}'
    ```
    """
    try:
        # Перетворення дат з рядкового формату у datetime
        start_date = datetime.strptime(request.date_start, "%Y%m%d")
        end_date = datetime.strptime(request.date_end, "%Y%m%d")

        processor = TelegramFactory.create_processor(
        ...
        for document in documents:
            ...
            data_for_mongo = {"id_telegram": id_telegram, "data": document}
            data_for_mongo["data"].pop("id_telegram", None)
            collection = db_manager.get_or_create_collection(request.country_code)
            db_manager.insert_or_update_document(collection, data_for_mongo)
            print(data_for_mongo)
        return {
            "message": f"Successfully downloaded telegrams for {request.country_code} from {request.date_start} to {request.date_end}"}
    except Exception as e:
        return {"error": str(e)}
